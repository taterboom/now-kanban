...
function MaterialSymbolsSettingsOutlineRounded(props: React.SVGProps<SVGSVGElement>) {
  return (
...
export function Settings(props: PropsWithChildren) {
  ...
  return (
    <div ref={el} className="relative z-10">
      <button
        className="absolute right-0 top-0 transition-opacity opacity-70 hover:opacity-100"
        onClick={() => {
          setVisible(!visible)
        }}
      >
      ...
      {visible && (
        <div className="absolute right-0 top-8 p-4 bg-base-300 rounded-2xl shadow-2xl">
          {props.children}
        </div>
      )}
    </div>
